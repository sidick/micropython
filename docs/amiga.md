...
Save the script below as `runtests.py` on the Amiga (e.g. on `RAM:` or a
volume) and run it to batch-test a whole directory. It uses
`amiga.execute()` with shell redirection to capture each test's output
and compares against `.exp` files where they exist.
...
# LF -> CRLF), but the .exp files generated by tools/amiga-gen-exp.py are
# LF-only to match CPython. Normalise both sides before comparing.
def _norm(s):
    return s.replace("\r\n", "\n").replace("\r", "\n").strip()
